// Test script to debug SPECIFIC channel message delete API endpoints
console.log('Testing CHANNEL-SPECIFIC group message delete endpoints...');

async function testChannelMessageDelete() {
  try {
    // Step 1: Login
    ...
    // Step 3: Get group channels
    console.log('3. Fetching group channels...');
    const channelsRes = await fetch(`http://localhost:3001/api/groups/${testGroup.group_id}/channels`, {
      headers: {
        'Cookie': sessionCookie || ''
      }
    });
    ...
    const testChannel = channelsData.channels[0];
    console.log('Using test channel:', testChannel);
    
    // Step 4: Get channel messages
    console.log('4. Fetching channel messages...');
    const messagesRes = await fetch(`http://localhost:3001/api/groups/${testGroup.group_id}/channels/${testChannel.channel_id}/messages`, {
      headers: {
        'Cookie': sessionCookie || ''
      }
    });
    ...
    if (!messagesData.success || !messagesData.messages || messagesData.messages.length === 0) {
      ...
      const sendRes = await fetch(`http://localhost:3001/api/groups/${testGroup.group_id}/channels/${testChannel.channel_id}/messages`, {
        method: 'POST',
        headers: { 
        ...
        body: JSON.stringify({
          content: 'Test message for channel deletion',
          attachments: []
        })
      });
      ...
      const messagesRes2 = await fetch(`http://localhost:3001/api/groups/${testGroup.group_id}/channels/${testChannel.channel_id}/messages`, {
        headers: {
          'Cookie': sessionCookie || ''
        }
      });
    ...
    // Step 5: Test channel-specific delete endpoint
    ...
    console.log('5. Testing CHANNEL-SPECIFIC delete endpoint for message:', testMessage.message_id);
    
    const deleteUrl = `http://localhost:3001/api/groups/${testGroup.group_id}/channels/${testChannel.channel_id}/messages`;
    ...
    if (deleteData.success) {
      console.log('✅ Channel-specific delete successful!');
    } else {
      console.log('❌ Channel-specific delete failed:', deleteData);
    }
