    ...
    <div className={`flex h-screen ${theme === 'dark' ? 'bg-black text-white' : 'bg-white text-black'}`}>
      ...
      <main className="flex-1 flex flex-col overflow-y-auto">
        <div className="w-full max-w-6xl mx-auto mt-4 md:mt-8 p-4 md:p-6 pb-8">
          <div className={`${theme === 'dark' ? 'bg-black border-white' : 'bg-white border-black'} border-2 rounded-none shadow-2xl mb-6`}>
            <div className="p-3 sm:p-6">
              {/* Category Selection */}
              <div className="flex flex-col sm:flex-row justify-center items-center gap-2 mb-4">
                <div className={`text-sm font-mono ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-2 sm:mb-0 sm:mr-4 self-center`}>Category:</div>
                    ...
                    <button
                      ...
                      className={`px-3 sm:px-4 py-2 border-2 font-mono text-xs sm:text-sm tracking-wider transition-all ${
                        selectedCategory === category 
                          ? `${categoryColors[category]} ${theme === 'dark' ? 'text-white' : 'text-black'}`
              ...
              {actionMessage && (
                <div className={`mb-4 text-center text-red-400 text-sm font-mono border border-red-400 ${theme === 'dark' ? 'bg-red-900/20' : 'bg-red-100'} p-2`}>
                  {actionMessage}
                </div>
              )}
          ...
          {/* Files Container */}
          <div className={`${theme === 'dark' ? 'bg-black border-white' : 'bg-white border-black'} border-2 rounded-none shadow-2xl`}>
            {loading ? (            <div className="text-center py-12">
              <div className={`font-mono ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-2`}>Loading...</div>
              <div className="text-red-400 animate-pulse">● ● ●</div>
            </div>
            ) : filteredItems.length === 0 ? (
              <div className="text-center py-12">
                <div className={`font-mono ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-4`}>
                  No {categoryLabels[selectedCategory]} profiles saved
                </div>
                <div className={`text-sm ${theme === 'dark' ? 'text-gray-500' : 'text-gray-600'} font-mono`}>
              ...
              <div className="p-3 sm:p-6">
                <div className="grid gap-4">
                  {filteredItems.map((item) => (
                    <div key={`${item.user_id}-${item.category}`} className={`border ${theme === 'dark' ? 'border-gray-600 bg-gray-900/50' : 'border-gray-400 bg-gray-100/50'} relative`}>
                      {/* File Header */}
                      <div className={`${theme === 'dark' ? 'bg-white text-black' : 'bg-black text-white'} p-2 font-mono text-xs flex flex-col sm:flex-row sm:justify-between`}>
                        <div>
                        ...
                        <div className="flex flex-col md:flex-row md:items-start md:justify-between">
                          ...
                          <div className="flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-4 flex-1">
                            ...
                            <div className={`border-2 ${theme === 'dark' ? 'border-white bg-gray-800' : 'border-black bg-gray-200'} p-2`}>
                              ...
                              <ProfileAvatar userId={item.user_id} size={64} className={`border ${theme === 'dark' ? 'border-gray-600' : 'border-gray-400'}`} />
                            </div>
                            ...
                            <div className="flex-1 font-mono w-full">
                              <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-2 text-sm">
                                ...
                                <div>
                                  ...
                                  <div className={`${theme === 'dark' ? 'text-white' : 'text-black'}`}>{categoryLabels[item.category]}</div>
                                </div>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex sm:flex-col justify-center mt-4 md:mt-0 space-x-2 sm:space-x-0 sm:space-y-2 sm:ml-4">
                            <button 
                              ...
                              className={`px-3 sm:px-4 py-2 ${theme === 'dark' ? 'bg-white text-black border-black hover:bg-gray-200' : 'bg-black text-white border-white hover:bg-gray-800'} font-mono text-xs border-2 transition-colors tracking-wider flex-1 sm:flex-none`}
                            >
                              View Profile
                            </button>
                            <button 
                              ...
                              className={`px-3 sm:px-4 py-2 font-mono text-xs border-2 transition-colors tracking-wider flex-1 sm:flex-none ${
                                item.ai_excluded 
                                  ? `${theme === 'dark' ? 'bg-gray-700 text-gray-500 border-gray-600' : 'bg-gray-300 text-gray-600 border-gray-400'} cursor-not-allowed` 
                                  : `${theme === 'dark' ? 'bg-blue-900 text-white border-blue-700 hover:bg-blue-800' : 'bg-blue-200 text-black border-blue-400 hover:bg-blue-300'} disabled:opacity-50 disabled:cursor-not-allowed`
                              }`}
                            ...
                            <button 
                              ...
                              className={`px-3 sm:px-4 py-2 ${theme === 'dark' ? 'bg-red-900 text-white border-red-700 hover:bg-red-800' : 'bg-red-200 text-black border-red-400 hover:bg-red-300'} font-mono text-xs border-2 transition-colors tracking-wider flex-1 sm:flex-none`}
                            >
                              Remove
                            </button>
                          </div>
        ...
        {showAIModal && aiAnalysis && (
          <div className={`fixed inset-0 ${theme === 'dark' ? 'bg-black' : 'bg-white'} bg-opacity-50 flex items-center justify-center z-50 p-4`}>
            <div className={`${theme === 'dark' ? 'bg-black border-white' : 'bg-white border-black'} border-2 rounded-none shadow-lg w-full max-w-4xl max-h-[90vh] overflow-hidden`}>
              {/* Modal Header */}
              <div className={`border-b-2 ${theme === 'dark' ? 'border-white bg-white text-black' : 'border-black bg-black text-white'} p-3 flex justify-between items-center`}>
                <div>
              ...
              <div className="p-4 md:p-6 overflow-y-auto max-h-[calc(90vh-80px)]">
                <div className="space-y-4">
                  {/* AI Analysis Content */}
                  <div className={`${theme === 'dark' ? 'bg-gray-900/30 border-gray-600' : 'bg-gray-100/50 border-gray-400'} border p-4`}>
                    <div className={`text-xs font-mono ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'} mb-3 flex items-center`}>
                      <span className="animate-pulse mr-2">🧠</span>
                      GROK AI ANALYSIS
                    </div>
                    <div className={`text-sm ${theme === 'dark' ? 'text-white' : 'text-black'} leading-relaxed`}>
                      {aiAnalysis.analysis.split('\n').map((line, index) => {
                          ...
                          return (
                            <p key={index} className="mb-3">
                              {parts.map((part, partIndex) => 
                                partIndex % 2 === 1 ? 
                                  <strong key={partIndex} className={`font-bold ${theme === 'dark' ? 'text-white' : 'text-black'}`}>{part}</strong> : 
                        ...
                        if (line.trim()) {
                          return (
                            <p key={index} className={`mb-3 ${theme === 'dark' ? 'text-gray-200' : 'text-gray-800'}`}>
                              {line}
                            </p>
                          );
                        }
